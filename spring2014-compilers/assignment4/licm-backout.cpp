...
namespace {
    ...
    class LoopICM : public FunctionPass, public AssemblyAnnotationWriter{

        public:
            static char ID;

            /* set forward false in the constructor for liveness */
            LoopICM() : FunctionPass(ID) {
            ...
            typedef ValueMap<const BasicBlock*, BitVector*> BlockInOutMap;
            BlockInOutMap *rdIn;
            BlockInOutMap *rdOut;

            class ReachingDef : public DataFlow<BitVector>{
                public:                    
                ReachingDef(int doSize, int nArgs, ValueMap<const Instruction*, BitVector*>& insInSet, 
                        std::vector<Value*>& bvIndex, ValueMap<Value*, int>& vToBv, BlockInOutMap *rdin, BlockInOutMap *rdout) : DataFlow<BitVector>(true) {
                    domainSize = doSize;
                    numArgs = nArgs;
                    instrInSet  = &insInSet;
                    bvIndexToInstrArg = &bvIndex;
                    valueToBitVectorIndex = &vToBv;
                    localrdin = rdin;
                    locardout = rdout;
                }
                typedef ValueMap<const BasicBlock*, BitVector*> BlockInOutMap;
                BlockInOutMap *localrdin;
                BlockInOutMap *locardout;
                int domainSize;                
                int numArgs;
            ...
            virtual bool runOnFunction(Function &F) {
                ...
                ReachingDef * rdef = new ReachingDef( domainSize, numArgs, *instrInSet, *bvIndexToInstrArg, *valueToBitVectorIndex, rdIn, rdOut );
                rdef->run(F);
//                ::runOnFunction(F); //call the analysis method in dataflow
